...
class OfflineService {
    ...
    async updateCard(tempId, cardData, imageBeforeFile, imageAfterFile) {
        try {
            const db = await this.getDB();
            return new Promise((resolve, reject) => {
                const transaction = db.transaction([STORE_NAME], 'readwrite');
                ...
                // First get existing to preserve what we don't change if needed, 
                // though we usually overwrite the data.
                ...
                getRequest.onsuccess = () => {
                    ...
                    const updatedRecord = {
                        ...
                        data: cardData,
                        files: {
                            imageBefore: imageBeforeFile !== undefined ? imageBeforeFile : existingRecord.files.imageBefore,
                            imageAfter: imageAfterFile !== undefined ? imageAfterFile : existingRecord.files.imageAfter
                        }
                    };
            ...
            console.error("OfflineService updateCard error:", error);
            throw error;
        }
    }
