...
const A3FiveWhys = ({ items = [], onChange }: A3FiveWhysProps) => {
    const handleAddRow = (parentId: number | null = null, parentWhyIndex: number = -1) => {
    ...
    const updateItem = (index: number, field: keyof FiveWhysItem, value: any) => {
    ...
    const toggleStatus = (index: number) => {
        const statuses: FiveWhysItem['status'][] = ['neutral', 'root', 'discarded'];
        const currentStatus = items[index].status || 'neutral';
        const nextStatus = statuses[(statuses.indexOf(currentStatus) + 1) % statuses.length];
        updateItem(index, 'status', nextStatus);
    };

    const getStatusStyles = (status: FiveWhysItem['status']) => {
        switch (status) {
            ...
            <div className="min-w-[1200px]">
                ...
                <div className="grid grid-cols-7 gap-6 mb-6 text-center px-2">
                    ...
                    {[1, 2, 3, 4, 5].map(i => (
                        <div key={i} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col justify-center h-24 relative overflow-hidden group">
                            <div className="absolute -right-2 -bottom-4 text-slate-100 font-black text-6xl group-hover:text-blue-50 transition-colors z-0 select-none">
                                {i}
                            </div>
                ...
                <div className="space-y-6 px-2">
                    {items.map((item, rowIdx) => (
                        <div key={item.id || rowIdx} className="grid grid-cols-7 gap-6 items-stretch relative group">
                            {/* Primary Cause Input */}
                            <div className="relative">
                                <textarea
                                    className="w-full h-full min-h-[140px] p-4 rounded-xl border-2 border-slate-200 bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-sm font-medium text-slate-700 resize-none shadow-sm transition-all outline-none"
                                    placeholder="Describe la causa directa observada..."
                                    value={item.problem || ''}
                                    onChange={(e) => updateItem(rowIdx, 'problem', e.target.value)}
                                />
                                <div className="absolute top-1/2 -right-3 sm:-right-4 transform -translate-y-1/2 z-10 text-blue-200">
                            ...
                            {[0, 1, 2, 3, 4].map((whyIdx) => (
                                <div key={whyIdx} className="relative">
                                    <textarea
                                        className={`w-full h-full min-h-[140px] p-4 rounded-xl border-2 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none text-sm text-slate-600 resize-none shadow-sm transition-all ${(item.whys || [])[whyIdx]
                                    ...
                                    <button
                                        className="absolute bottom-2 right-2 z-10 bg-white rounded-lg p-1.5 shadow-sm border border-slate-200 text-slate-400 hover:text-blue-600 hover:border-blue-200 transition-all opacity-0 group-hover:opacity-100 active:scale-95"
                            ...
                            <div className="relative">
                                <div
                                    className={`rounded-xl border-2 p-2 flex flex-col justify-center items-center gap-3 cursor-pointer transition-all shadow-sm h-full w-full relative group/status select-none ${getStatusStyles(item.status)}`}
                                ...
                                >
                                    ...
                                    {(!item.status || item.status === 'neutral') && (
                                        <>
                                            <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 group-hover/status:text-blue-400 group-hover/status:bg-blue-50 transition-colors">
                                                <HelpCircle size={28} />
                                            </div>
                                            <span className="text-[10px] font-bold text-slate-400 uppercase text-center leading-none group-hover/status:text-blue-500">Evaluar</span>
                                        </>
                                    )}
                                </div>
                ...
                <div className="mt-8 px-2">
                    <button
                        ...
                        className="w-full border-2 border-dashed border-slate-300 rounded-xl p-4 text-slate-400 font-bold hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50/50 transition-all flex items-center justify-center gap-2 group"
                    >
