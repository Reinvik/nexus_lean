...
const PRIORITIES: Record<PriorityColor, { color: string; ring: string; label: string; next: PriorityColor; shadow: string }> = {
...
const A3Ishikawa = ({ data, onChange, onDelete, index }: A3IshikawaProps) => {
    const [isFullScreen, setIsFullScreen] = useState(false);

    const getCauseData = (cause: string | Cause): Cause => {
    ...
    const handleAddCause = (catId: string) => {
        ...
        if (causeText) {
            const newCategories = {
                ...data.categories,
                [catId]: [...currentCauses, { text: causeText, color: 'neutral' as PriorityColor }]
            };
    ...
    return (
        <div className={`
            rounded-2xl overflow-hidden bg-white shadow-sm border border-slate-200 transition-all
        ...
        `}>
            ...
            <div className={`p-6 bg-slate-50/50 relative overflow-auto ${isFullScreen ? 'flex-1 flex items-center justify-center' : ''}`}>
                <div className={`min-w-[900px] max-w-full mx-auto relative pb-10 pt-2 ${isFullScreen ? 'scale-110 transform origin-center' : ''}`}>
                    ...
                    <div className="absolute top-1/2 right-24 transform -translate-y-1/2 z-20 w-[240px] h-[180px] flex items-center">
                        <div className="bg-white border-2 border-slate-200 shadow-xl rounded-2xl p-6 w-full h-full flex flex-col justify-center items-center relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-3 gap-x-12 gap-y-16 pr-[380px] relative z-10 ml-10">
                        ...
                        {CATEGORIES.slice(0, 3).map((cat) => (
                            <div key={cat.id} className="h-56 flex flex-col justify-end items-center relative group px-4">
                                {/* Diagonal Rib Line */}
                                <div className="absolute bottom-0 left-1/2 w-0.5 h-[120%] bg-slate-300 origin-bottom transform -rotate-[30deg] group-hover:bg-blue-200 transition-colors"></div>
                                ...
                                <div className="z-20 w-full flex flex-col gap-2 items-end pr-6">
                                    ...
                                    <button
                                        ...
                                        className="self-end mt-1 text-slate-400 hover:text-blue-600 hover:bg-blue-50 border border-dashed border-slate-300 hover:border-blue-200 rounded-full p-1.5 transition-all"
                                        title="Agregar Causa"
                                    >
                        ...
                        {CATEGORIES.slice(3, 6).map((cat) => (
                            <div key={cat.id} className="h-56 flex flex-col justify-start items-center relative group px-4 mt-4">
                                {/* Diagonal Rib Line */}
                                <div className="absolute top-0 left-1/2 w-0.5 h-[120%] bg-slate-300 origin-top transform rotate-[30deg] group-hover:bg-blue-200 transition-colors"></div>
                    ...
                    <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-end max-w-4xl mx-auto">
                        ...
                        <div className="bg-blue-50/50 rounded-xl p-4 border border-blue-100 shadow-inner">
                            ...
                            <div className="relative">
                                <select
                                    className="w-full text-sm p-2.5 border border-blue-200 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 outline-none shadow-sm cursor-pointer text-black font-medium"
