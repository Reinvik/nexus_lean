...
import { Plus, X, AlertCircle, Info, ChevronRight, Fish, Circle, ArrowRight } from 'lucide-react';
...
const A3Ishikawa = ({ data, onChange, onDelete, index }) => {
    ...
    return (
        <div className="card animate-in fade-in zoom-in-95 duration-300 rounded-2xl overflow-hidden bg-white shadow-sm border border-slate-200">
            {/* Header */}
            <div className="bg-slate-900 px-6 py-4 flex justify-between items-center text-white relative overflow-hidden">
                ...
                {onDelete && (
                    <button onClick={onDelete} className="relative z-10 text-rose-300 hover:text-white hover:bg-rose-500/20 px-3 py-1.5 rounded-lg text-xs font-medium flex items-center gap-2 transition-all active:scale-95">
                        <X size={14} /> Eliminar
                    </button>
                )}
            </div>

            <div className="p-8 bg-slate-50/50 relative overflow-x-auto">
                <div className="min-w-[900px] max-w-full mx-auto relative pb-10 pt-4">

                    {/* Main Spine */}
                    <div className="absolute top-1/2 left-0 right-[240px] h-1 bg-slate-400 rounded-full z-0 shadow-sm"></div>
                    <div className="absolute top-1/2 right-[240px] w-4 h-4 rounded-full bg-slate-400 -mt-1.5 z-0"></div>

                    {/* Fish Head (Problem) */}
                    <div className="absolute top-1/2 right-0 transform -translate-y-1/2 z-20 w-[240px] h-[180px] flex items-center">
                        <div className="bg-white border-2 border-slate-200 shadow-xl rounded-2xl p-6 w-full h-full flex flex-col justify-center items-center relative overflow-hidden group">
                            ...
                            <label className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-3">Problema Central</label>
                            <textarea
                                className="w-full h-full bg-transparent border-none text-slate-800 placeholder-slate-300 text-sm font-bold text-center focus:ring-0 resize-none overflow-hidden leading-relaxed"
                                rows="4"
                    ...
                    {/* Ribs Container */}
                    <div className="grid grid-cols-3 gap-x-12 gap-y-24 pr-[260px] relative z-10 ml-10">
                        {/* Top Ribs */}
                            ...
                            <div key={cat.id} className="h-56 flex flex-col justify-end items-center relative group px-4">
                                ...
                                <div className="z-20 w-full flex flex-col gap-2 items-end pr-6">
                                    {(data.categories?.[cat.id] || []).map((cause, cIdx) => {
                                        ...
                                        return (
                                            <div key={cIdx} className="relative group/chip bg-white border border-slate-200 text-slate-700 text-xs pl-2 pr-2 py-1.5 rounded-lg shadow-sm hover:shadow-md transition-all w-full flex items-center justify-between gap-2 hover:-translate-y-0.5">
                                                <span className="flex-grow text-right break-words leading-tight text-[11px] font-medium">{text}</span>
                                                <button
                ...
                <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-end max-w-4xl mx-auto">
                    ...
                    <div className="bg-blue-50/50 rounded-xl p-4 border border-blue-100 shadow-inner">
                        <label className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-2 block">Seleccionar Causa Raíz Principal</label>
                        <div className="relative">
                            <select
                                className="w-full text-sm p-2.5 border border-blue-200 rounded-lg bg-white focus:ring-2 focus:ring-brand-500 outline-none shadow-sm cursor-pointer"
