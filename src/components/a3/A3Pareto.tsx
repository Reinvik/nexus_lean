...
interface A3ParetoProps {
    data: ParetoItem[];
    onChange: (data: ParetoItem[]) => void;
}

const A3Pareto = ({ data = [], onChange }: A3ParetoProps) => {
    const [items, setItems] = useState<ParetoItem[]>(data);
    ...
    const calculateParetoData = (rawData: ParetoItem[]) => {
                ...
                <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 lg:col-span-1">
                    ...
                    <div className="space-y-4 mb-6">
                        <div>
                            ...
                            <input
                                type="text"
                                className="w-full p-2 border border-slate-300 rounded-lg text-sm text-slate-900 font-medium focus:ring-2 focus:ring-blue-500 outline-none"
                        ...
                        <button
                            ...
                            className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg text-sm transition-colors flex items-center justify-center gap-2"
                        >
                        ...
                        {items.length === 0 ? (
                            ...
                            items.map((item, idx) => (
                                <div key={idx} className="flex justify-between items-center bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                                    ...
                                    <span className="text-sm font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded mx-2">{item.value}</span>
                                    <button
            ...
            {items.length > 0 && (
                <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg">
                    ...
                    <p className="text-sm text-blue-700">
                        Las categorías <strong>{chartData.filter(i => (i.cumulativePercentage || 0) <= 80).map(i => i.name).join(', ')}</strong> representan el ~80% del impacto total.
                    </p>
                </div>
            )}
        </div>
